...
// `export * from "<file>"` only. Per ES module semantics, `export *` never
// re-exports a module's `default` binding — so a repo whose components are
// all `export default function Name(...)` (this one, 9/9) never lands a
// single component on window.<Global>: package-validate.mjs then fails
// every component with [BUNDLE_EXPORT]. Fixed below by also emitting an
// `export { default as Name } from "<file>"` alias per file, using the same
// ts-morph "recover the real name behind `default`" technique
// deriveComponentsFromSrc already uses for component-list discovery.
//
// Second fix, same block: `cfg.componentSrcMap.<Name>: null` previously only
// filtered the discovered component LIST, downstream of barrel generation —
// an excluded file's default export was still bundled (unnamed, via the
// original `export *`). For Button/Marquee (excluded because next/link and
// next/image throw `ReferenceError: process is not defined` outside a real
// Next.js runtime — see NOTES.md), that meant the broken code still shipped
// inside _ds_bundle.js and, since every component shares one IIFE, a
// module-init failure there could take every other component down with it.
// Now `componentSrcMap: null` skips the file at the barrel stage too.
//
...
// Resolve the real declared name behind `export default` in one file (or
// null — no default export, or an unnamed/non-component default). Same
// recovery technique as deriveComponentsFromSrc below, scoped to one file
// so the barrel can emit a per-file alias.
function defaultExportName(project, filePath) {
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    const barrelProject = new Project({
      skipAddingFilesFromTsConfig: true,
      compilerOptions: { jsx: ts.JsxEmit.Preserve, allowJs: true, skipLibCheck: true },
    });
    // A `componentSrcMap.<Name>: null` exclusion must keep the file OUT of
    // the bundle graph entirely, not just out of the discovered component
    // list below — otherwise an excluded file's module-level side effects
    // (e.g. a hard runtime dependency the file was excluded FOR) still ship
    // inside _ds_bundle.js and can break init for every other component in
    // the same IIFE. Skip such files before emitting any barrel line.
    const barrelLines = comps.flatMap((p) => {
      const name = defaultExportName(barrelProject, p);
      if (name && srcMap[name] === null) return [];
      const lines = [`export * from ${JSON.stringify(p)};`];
      if (name) lines.push(`export { default as ${name} } from ${JSON.stringify(p)};`);
      return lines;
    });
    writeFileSync(entry, barrelLines.join('\n') + '\n');
    synthEntry = true;
    console.error(
