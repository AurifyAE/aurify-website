...
/**
 * Complete list of countries for the phone number selector.
 */
...
/**
 * Renders a crisp SVG flag icon for a 2-letter ISO country code.
 */
export function FlagIcon({
  code,
  className = "h-4 w-5.5 rounded-[2px] object-cover shadow-sm",
}: {
  code: string;
  className?: string;
}) {
  ...
  if (!FlagComponent) {
    return React.createElement(
      "span",
      { className: "inline-block h-4 w-5.5 rounded-[2px] bg-ink/10 text-center text-xs" },
      "🌐"
    );
  }
