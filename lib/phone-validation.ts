...
export function validatePhoneForCountry(
  ...
  if (!/^[0-9\s()\-]+$/.test(phone)) {
    return { error: `Enter a valid phone number for ${country.name}.` };
  }
  ...
  if (lengthProblem === "TOO_SHORT") {
    return { error: `The phone number is too short for ${country.name}.` };
  }

  if (lengthProblem === "TOO_LONG") {
    return { error: `The phone number is too long for ${country.name}.` };
  }

  if (lengthProblem) {
    return { error: `Enter a phone number with a valid length for ${country.name}.` };
  }
